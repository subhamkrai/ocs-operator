...
var _ k8sReader = (*clientsetReader)(nil)
...
func (c *Conn) fetchCredentials() (*cephCredentials, error) {
	...
	secret, err := c.k8s.getSecret(ctx, c.ns, util.OcsMetricsExporterCephClientName)
	if err != nil {
	...
	configmap, err := c.k8s.getConfigMap(ctx, c.ns, monEndpointConfigMap)
	if err != nil {
