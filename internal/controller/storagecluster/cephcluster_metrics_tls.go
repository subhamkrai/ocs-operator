...
func configureCephClusterMetricsTLS(sc *ocsv1.StorageCluster, cephCluster *rookCephv1.CephCluster) {
	if sc.Spec.Monitoring == nil || !sc.Spec.Monitoring.EnableCephMetricsTLS || sc.Spec.ExternalStorage.Enable {
		cephCluster.Spec.Monitoring.MetricsTLS = nil
		return
	}
	cephCluster.Spec.Monitoring.MetricsTLS = &rookCephv1.MetricsTLSSpec{
		Enabled:    true,
		SecretName: rookCephv1.DefaultMetricsTLSSecretName,
	}
}
...
func (r *StorageClusterReconciler) ensureCephMgrMetricsServiceServingCert(ctx context.Context, sc *ocsv1.StorageCluster) error {
	...
	secretName := rookCephv1.DefaultMetricsTLSSecretName
	if svc.Annotations != nil && svc.Annotations[rookCephv1.ServiceServingCertKey] == secretName {
		return nil
	}
	if svc.Annotations == nil {
		svc.Annotations = map[string]string{}
	}
	svc.Annotations[rookCephv1.ServiceServingCertKey] = secretName
